...
grid = '/lus/work/CT1/c1601279/lweiss/CROCO/RUN/SWIOSE/CROCO_FILES/grid/croco_grid_swio_v2.nc'
simu = 'run_swio_test_new_grid'
...
ds = xr.open_dataset(work + '/RUN/SWIOSE/OUTPUT/' + simu + '/swiose_his.nc') # , engine='h5netcdf')
temp = ds['temp'][:, :, :, :]
...
### SAVE
plt.savefig(path_fig + '/bathy_iso_swio_v2_grid.png', dpi=300, bbox_inches='tight')
# plt.show()
plt.close()
sys.exit()
...
plt.savefig(path_fig + 'transect_h_lon_' + str(np.round(lon[lat_index, lon_index].values,2)) + '_' + simu + '.png', dpi=300, bbox_inches='tight')
# plt.show()
...
ax.set_ylabel('Depth h (m)')
plt.grid()
plt.savefig(path_fig + 'transect_h_lat_' + str(np.round(lat[lat_index, lon_index].values,2)) + '_' + simu + '.png', dpi=300, bbox_inches='tight')
