...
zeta_bias = zeta_mean1 - zeta_mean2

### spatial mean bias
spatial_mean = zeta_bias.mean(dim=['eta_rho','xi_rho'], skipna=True)
print(f'spatial mean: {spatial_mean.values}')
### correlation coefficient
m1_flat = zeta_mean1.values.flatten()
m2_flat = zeta_mean2.values.flatten()
mask = ~np.isnan(m1_flat) & ~np.isnan(m2_flat)
m1_flat_ed = m1_flat[mask]
m2_flat_ed = m2_flat[mask]
corr_coeff, p_val = pearsonr(m1_flat_ed, m2_flat_ed)
print(f'Corr Coeff (R): {corr_coeff}')
...
cmap = cmcrameri.cm.vik #roma_r #vik
a = -0.8 #-0.2 # -0.1
b = 0.8 #1.2 # 0.1
c = 10 #15 # 10 # int((b - a) + 1)
levels = np.linspace(a, b, c)  ### amplitude de la colorbar à ajuster ici
norm = mpl.colors.BoundaryNorm(levels, cmap.N)

pcm = ax.pcolormesh(lon, lat, zeta_bias, cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
...
ax.text(0.02, 0.98, f'R={corr_coeff:.2f}, p-val<0.01\nmean bias={spatial_mean.item():.3f}', transform=ax.transAxes, fontsize=9, verticalalignment='top', bbox=dict(boxstyle="round,pad=0.3",edgecolor='black',facecolor='white',alpha=0.5))
...
ax.set_title(f"SSH bias - {start_time[:13]}", size=9)

### colorbar
cb = fig.colorbar(pcm, ax=ax, label='Sea Surface Height Bias [m]')
colorbaryticks = np.linspace(a, b, 5) #int(c/2)+1) # 5  # levels[::2]
...
### SAVE
plt.savefig(path_fig + simu + f'/ssh_bias_points_{simu}_{start_time[:13]}.png', dpi=300, bbox_inches='tight')
# plt.show()
plt.close()
