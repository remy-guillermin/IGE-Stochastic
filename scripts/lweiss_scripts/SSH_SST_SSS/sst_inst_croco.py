...
### plot
for t in range(0,temp.shape[0]+1):
    ...
    pcm = ax.pcolormesh(lon, lat, temp[t], cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
    ...
    ax.set_title(f"Daily mean SST {t_title[:10]}", size=10)
    ...
    ### SAVE
    plt.savefig(path_fig + simu + f'/sst/sst_{simu[:13]}_{time_str_formatted[:10]}.png', dpi=300, bbox_inches='tight')
    # plt.show()
    plt.close()
# sys.exit()
