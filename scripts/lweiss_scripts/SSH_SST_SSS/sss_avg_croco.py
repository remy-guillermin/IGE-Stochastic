...
salt = salt.where((salt != fill_value) & (ds.mask_rho == 1), np.nan)
...
salt_mean = salt_period.mean(dim='time', skipna=True)

# Sauvegarder temp_mean comme NetCDF
salt_mean.to_netcdf(path_fig + simu + f'/Mean_SSS/sss_mean_{simu}_{start_time}_{end_time}.nc')
...
### SAVE
plt.savefig(path_fig + simu + f'/sss_mean_{simu}_{start_time}_{end_time}.png', dpi=300, bbox_inches='tight')
plt.close()
