...
print('U, V, vorticity, lon, lat, msk \n', u.shape, v.shape, len(vorticities), lon.shape, lat.shape, msk.shape)

### plot
for t in range(0, len(vorticities)):
    ...
    pcm = ax.pcolormesh(lon, lat, vorticities[t]*10000, cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
    ...
    ax.set_title(f"Surface vorticity {t_title[:10]}", size=10)
    ...
    ### SAVE
    plt.savefig(path_fig + simu + f'/vorticity/vorticity_{simu[:13]}_{time_str_formatted[:10]}.png', dpi=300, bbox_inches='tight')
    # plt.show()
    plt.close()
# sys.exit()
