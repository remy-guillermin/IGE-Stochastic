...
uu = units.Quantity(u_fluct[:,:,:].data, "m/s")
vv = units.Quantity(v_fluct[:,:,:].data, "m/s")
...
ax.set_title(f"EKE GLORYS {start_time} to {end_time}", size=10)
...
### SAVE
plt.savefig(path_fig + simu + f'/eke_mean_{simu[:13]}_{start_time}_{end_time}.png', dpi=300, bbox_inches='tight')
# plt.show()
plt.close()
