...
uu = units.Quantity(u_mean[:-1,:].data, "m/s")
vv = units.Quantity(v_mean[:,:-1].data, "m/s")

mke = 0.5 * (uu ** 2 + vv ** 2)

# mke_mean = np.nanmean(mke, axis=0)

print('U, V, mke, lon, lat, msk \n', u_period.shape, v_period.shape, mke.shape, lon.shape, lat.shape, msk.shape)
...
pcm = ax.pcolormesh(lon, lat, mke, cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
...
ax.set_title(f"MKE {start_time} to {end_time}", size=10)
...
### SAVE
plt.savefig(path_fig + simu + f'/mke_mean_box_{simu[:13]}_{start_time}_{end_time}.png', dpi=300, bbox_inches='tight')
# plt.show()
plt.close()
