...
data = '/lus/scratch/CT1/c1601279/lweiss/CROCO/SWIO/' # '/lus/store/CT1/c1601279/lweiss/RUN_CROCO/'
work = '/lus/work/CT1/c1601279/lweiss/CROCO/'
path_fig = '/lus/home/CT1/c1601279/lweiss/PYTHON/FIGURES/'

simu = 'run_swio2_deter_2017'
...
u_mean = u_period.mean(dim='time')
v_mean = v_period.mean(dim='time')

u_fluct = u_period - u_mean
v_fluct = v_period - v_mean
# ds.close()

uu = units.Quantity(u_fluct[:,:-1,:].data, "m/s")
vv = units.Quantity(v_fluct[:,:,:-1].data, "m/s")

eke = 0.5 * (uu ** 2 + vv ** 2)

eke_mean = np.nanmean(eke, axis=0)

print('U, V, eke, lon, lat, msk \n', u_fluct.shape, v_fluct.shape, eke_mean.shape, lon.shape, lat.shape, msk.shape)
...
pcm = ax.pcolormesh(lon, lat, eke_mean, cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
...
ax.set_title(f"EKE {start_time} to {end_time}", size=10)
...
### colorbar
cb = fig.colorbar(pcm, ax=ax, label='Eddy Kinetic Energy $[m^{2} s^{-2}]$')
...
### SAVE
plt.savefig(path_fig + simu + f'/eke_mean_box_{simu[:13]}_{start_time}_{end_time}_bis.png', dpi=300, bbox_inches='tight')
# plt.show()
plt.close()
