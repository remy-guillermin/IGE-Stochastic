...
print('U, V, eke, lon, lat, msk \n', u_fluct.shape, v_fluct.shape, eke.shape, lon.shape, lat.shape, msk.shape)

### plot
for t in range(0, eke.shape[0]):
    ...
    pcm = ax.pcolormesh(lon, lat, eke[t], cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
    ...
    ax.set_title(f"Daily EKE {t_title[:10]}", size=10)
    ...
    ### SAVE
    plt.savefig(path_fig + simu + f'/eke/eke_{simu[:13]}_{time_str_formatted[:10]}.png', dpi=300, bbox_inches='tight')
    # plt.show()
    plt.close()
