...
ds = xr.open_dataset('swio_avg_pt1.nc')
u = ds.u[:,-1:,:,:]
v = ds.v[:,-1:,:,:]
w = ds.w[:,-1:,:,:]
salt = ds.salt[:,-1:,:,:]
...
ds = xr.open_dataset('swio_avg_pt2.nc')
u2 = ds.u[:,-1:,:,:]
v2 = ds.v[:,-1:,:,:]
w2 = ds.w[:,-1:,:,:]
salt2 = ds.salt[:,-1:,:,:]
...
u_tot = xr.concat([u, u2], dim='time')
print('u concatenated')
v_tot = xr.concat([v, v2], dim='time')
print('v concatenated')
w_tot = xr.concat([w, w2], dim='time')
print('w concatenated')
salt_tot = xr.concat([salt, salt2], dim='time')
print('salinity concatenated')
temp_tot = xr.concat([temp, temp2], dim='time')
print('temperature concatenated')
zeta_tot = xr.concat([zeta, zeta2], dim='time')
